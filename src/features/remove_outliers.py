...
df[["acc_x", "exercise"]].boxplot(by="exercise", figsize=(20, 10))

df[outlier_columns[:3] + ["exercise"]].boxplot(
    by="exercise", figsize=(20, 10), layout=(1, 3)
)
df[outlier_columns[3:] + ["exercise"]].boxplot(
    by="exercise", figsize=(20, 10), layout=(1, 3)
)
...
df[outlier_columns[:3] + ["exercise"]].plot.hist(
    by="exercise", figsize=(20, 20), layout=(3, 3)
)
df[outlier_columns[3:] + ["exercise"]].plot.hist(
    by="exercise", figsize=(20, 20), layout=(3, 3)
)
...
dataset, outliers, X_scores = mark_outliers_lof(
    df[df["exercise"] == exercise], outlier_columns
)
for col in outlier_columns:
    plot_binary_outliers(
        dataset=dataset, col=col, outlier_col="outlier_lof", reset_index=True
...
dataset[dataset["gyr_z_outlier"]]  # boolean indexing used same as == True
# not using explicit comparison, USED IMPLICIT COMPARISON

dataset.loc[dataset["gyr_z_outlier"], "gyr_z"] = np.nan  # updates inplace
...
for col in outlier_columns:
    for exercise in df["exercise"].unique():
        ...
        outliers_removed_df.loc[(outliers_removed_df["exercise"] == exercise), col] = (
            dataset[col]
        )
